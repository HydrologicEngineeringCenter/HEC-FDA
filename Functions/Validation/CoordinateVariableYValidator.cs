...
namespace Functions.Validation
{
    internal class CoordinateVariableYValidator : IValidator<Coordinates.CoordinateVariableY>
    {
        ...
        public IEnumerable<IMessage> ReportErrors(CoordinateVariableY obj)
        {
            ...
            if (obj.X.Messages.Any()) msgs.Add(IMessageFactory.Factory(obj.X.Messages.Max(), 
                $"The {obj.Print()} X {nameof(IOrdinate)} contains validation messages.",
                $"The following are {nameof(IOrdinate)} validation messages: \r\n {obj.X.Messages.PrintTabbedListOfMessages()}"));
            if (obj.Y.Messages.Any()) msgs.Add(IMessageFactory.Factory(obj.Y.Messages.Max(), 
                $"The {obj.Print()} Y {nameof(IOrdinate)} contains validation messages.",
