...
namespace Utilities.Messages
{
    internal class MessageBoard : IMessageBoard
    {
        ...
        {
            foreach (IMessage msg in messages) if(msg.Level > IMessageLevels.NotSet && msg.Notice.Length > 0) _OtherMessages.Enqueue(msg);
        }
        public void PostMessage(IMessage message)
        {
            if (!message.IsNull() && message.Level > IMessageLevels.NotSet && message.Notice.Length > 0) _OtherMessages.Enqueue(message);
        }
    }
}
