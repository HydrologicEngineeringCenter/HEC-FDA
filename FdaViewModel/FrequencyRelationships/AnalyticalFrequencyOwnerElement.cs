...
{
    class AnalyticalFrequencyOwnerElement : Utilities.OwnerElement
    {
        #region Notes
        #endregion
        #region Fields
        ...
        #endregion
        #region Constructors
        public AnalyticalFrequencyOwnerElement(BaseFdaElement owner) : base(owner)
        {
        ...
        public void AddNewFlowFrequencyCurve(object arg1, EventArgs arg2)
        {
            ...
            if (!vm.WasCancled)
            {
                if (!vm.HasError)
                {
                    ...
                    AnalyticalFrequencyElement afe = new AnalyticalFrequencyElement(vm.Name, vm.Description, lpiii, this);
                    AddElement(afe);
        ...
        {
            return new string[] { "Analyitical Frequency Curve", "Description", "Mean (of Log)", "Standard Deviation (of Log)", "Skew (of Log)", "Equivalent Years of Record" };
        }
        public override Type[] TableColumnTypes()
        {
            return new Type[] { typeof(string), typeof(string), typeof(double), typeof(double), typeof(double), typeof(int) };
        }
        public override void AddElement(object[] rowData)
        {
            double mean = (double)rowData[2];
            double stdev = (double)rowData[3];
            double skew = (double)rowData[4];
            Int64 n = (Int64)rowData[5];
            AddElement(new AnalyticalFrequencyElement((string)rowData[0], (string)rowData[1], new Statistics.LogPearsonIII(mean, stdev, skew, (int)n), this),false);
        }
        #endregion
    }
}
