...
namespace DbMemory
{
    class StrucGroupLookupDataTable
    {
        public void ProcessStrucGroupLookDataTable(DataTable dataTable)
        {
            StrucGroupLookupList theFuncList = GlobalVariables.mp_fdaStudy.GetStrucGroupLookupList();

            //Process Each Record (Row) in _dataTable and create new Prob function and Add
            for (int irec = 0; irec < dataTable.Rows.Count; irec++)
            {
                StrucGroupLookup theFunc = new StrucGroupLookup();
