...
namespace DbMemory
{
    class EadResultsLookupDataTable
    {
        public void ProcessEadLookDataTable(DataTable dataTable)
        {
            EadResultsLookupList theFuncList = GlobalVariables.mp_fdaStudy.GetEadResultsLookupList();

            //Process Each Record (Row) in _dataTable and create new Prob function and Add
            for (int irec = 0; irec < dataTable.Rows.Count; irec++)
            {
                EadResultsLookup theFunc = new EadResultsLookup();
