...
where T : struct
    ...
    /// <summary>
    /// returns all the component files of the terrain. Does not gaurantee they exist, just that they should for the terrain to be complete. For .hdf terrains  
    /// </summary>
    /// <param name="terrainPath"></param>
    /// <returns>List of files which comprise the terrain</returns>
    ...
    /// <summary>
    /// Checks that if the terrain is a .hdf, that all the component files exist.If it's anything else, just confirms <param name="path"></param> itself exists. 
    /// </summary>
    ...
    public static bool TerrainIsValid(string path, ref string error)
    {
        ...
        List<string> list = GetTerrainComponentFiles(path);
        foreach(string file in list)
        {
            if (!File.Exists(file))
            {
                error += $" {file} does not exist. {Environment.NewLine}";
                isValid = false;
            }
        }
        return isValid;
    ...
    #region HACKS
    /// <summary>
    /// This contains a workaround for an issue in RASMapper, where to query the component files of an HDF, RASMapper was generating a VRT using GDAL .exes that don't existin the version 7 build of GDAL 
    /// </summary>
