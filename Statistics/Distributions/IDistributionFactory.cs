...
namespace HEC.FDA.Statistics.Distributions
{
    ...
    public static class IDistributionFactory
    {
        ...
        public static string PrintParamaterizationRequirements(IDistributionEnum type)
        {
            switch (type)
            {
                ...
                case IDistributionEnum.LogPearsonIII:
                    return LogPearson3.RequiredParameterization(true);
                case IDistributionEnum.Normal:
                    return Normal.RequiredParameterization(true);
                case IDistributionEnum.Triangular:
                    return Triangular.RequiredParameterization(true);
                case IDistributionEnum.Uniform:
                    return Uniform.RequiredParameterization(true);
        ...
        internal static Histograms.Histogram Fit(IEnumerable<double> sample, int nBins)
        {
            double min = sample.Min();
            double max = sample.Max();
            double binWidth = (min - max) / nBins;
            Histograms.Histogram histogram = new Histograms.Histogram(binWidth);
        ...
        {
            return new Normal(mean, stDev, sampleSize);
        }
        /// <summary>
        ...
        {
            return new Triangular(min, mostLikely, max, sampleSize);
        }
        /// <summary>
        ...
        {
            return new Uniform(min, max, sampleSize);
        }
        /// <summary>
        ...
        /// <param name="sampleSize"> An optional sample size parameter. If a population rather than sample distribution is intended leave this parameter blank. Set to <see cref="int.MaxValue"/> by default. </param>
        /// <returns> A new <see cref="IDistribution"/>. </returns>
        public static IDistribution FactoryTruncatedNormal(double mean, double stDev, double min, double max, long sampleSize = int.MaxValue)
        {
            IDistribution normal = new TruncatedNormal(mean, stDev, min, max, sampleSize);
            return normal;
        }
        /// <summary>
        ...
        /// <returns> A <see cref="LogPearson3"/> object returned as an implementation of the  <see cref="IDistribution"/> interface. </returns>
        public static IDistribution FactoryLogPearsonIII(double mean, double stDev, double skew, int sampleSize = int.MaxValue)
        {
            return new LogPearson3(mean, stDev, skew, sampleSize);
        }
        /// <summary>
        ...
        /// <param name="min"> The lower (minimum) truncation value. </param>
        /// <param name="max"> The upper (maximum) truncation value. </param>
        /// <returns> A <see cref="LogPearson3"/> object bound on the range: [<paramref name="min"/>, <paramref name="max"/>] returned as an implementation of the  <see cref="IDistribution"/> interface. </returns>
        /// <returns></returns>
        ...
        {
            IDistribution lpIII = new LogPearson3(mean, stDev, skew, sampleSize);
            return FactoryTruncatedLogPearsonIII(lpIII, min, max);
        }
        /// <summary>
        /// Constructs a <see cref="IDistributionEnum.LogPearsonIII"/> <see cref="IDistribution"/> bound on the range specified by the <paramref name="min"/> and <paramref name="max"/> values."/>
        /// </summary>
        /// <param name="lpIII"> The <see cref="LogPearson3"/> distribution to be bound. </param>
        /// <param name="min"> The lower (minimum) truncation value. </param>
        /// <param name="max"> The upper (maximum) truncation value. </param>
        /// <returns> A <see cref="LogPearson3"/> object bound on the range: [<paramref name="min"/>, <paramref name="max"/>] returned as an implementation of the  <see cref="IDistribution"/> interface. </returns>
        public static IDistribution FactoryTruncatedLogPearsonIII(IDistribution lpIII, double min = 0, double max = double.PositiveInfinity)
        {
            ...
            LogPearson3 ldist = (LogPearson3)lpIII;
            return new TruncatedLogPearson3(ldist.Mean, ldist.StandardDeviation, ldist.Skewness, min, max, lpIII.SampleSize);
        }
        /// <summary>
        ...
        public static IDistribution FactoryTruncatedDistribution(IDistribution distribution, double min = double.NegativeInfinity, double max = double.PositiveInfinity)
        {
            ...
            {
                case IDistributionEnum.Normal:
                    Normal ndist = (Normal)distribution;
