...
namespace HEC.FDA.Statistics.Histograms
{
    public class Histogram : IHistogram, IReportMessage
    {
        ...
        public Histogram(double min, double binWidth)
        {
            ...
            Max = Min + _BinWidth;
            int numberOfBins = 1;
            _BinCounts = new long[numberOfBins];
            _ConvergenceCriteria = new ConvergenceCriteria();
            MessageHub.Register(this);
        }
        ...
        private Histogram(double min, double max, double binWidth, long sampleSize, long[] binCounts, ConvergenceCriteria convergenceCriteria)
        {
            Min = min;
            Max = max;
            _BinWidth = binWidth;
        ...
        public void AddObservationToHistogram(double observation, long index = 0) //TODO index is a hack
        {
            if (_SampleSize == 0)
            {
        ...
        public static Histogram ReadFromXML(XElement element)
        {
            ...
            long sampleSize = Convert.ToInt64(sampleSizeString);
        ...
        public long EstimateIterationsRemaining(double upperq, double lowerq)
        {
            ...
            double ufxp = PDF(uxp);
            long upperestimate = _ConvergenceCriteria.MaxIterations;
            if (ufxp > 0.0 & uxp != 0)
            {
                ...
                {
                    upperestimate = Math.Abs((long)estimate);
                }
            }
            double lp = lowerq;
            double lval = lp * (1 - lp);
            ...
            double lfxp = PDF(lxp);
            long lowerestimate = _ConvergenceCriteria.MaxIterations;
            if (lfxp > 0.0 & uxp != 0)
            {
                ...
                {
                    lowerestimate = Math.Abs((long)estimate);
                }
            }
            long biggestGuess = Math.Max(upperestimate, lowerestimate);
            long remainingIters = _ConvergenceCriteria.MaxIterations - _SampleSize;
