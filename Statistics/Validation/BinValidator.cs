...
namespace Statistics.Validation
{
    public class BinValidator: IValidator<IBin>
    {
        ...
        public IEnumerable<IMessage> ReportErrors(IBin obj)
        {
            ...
            if (!obj.MidPoint.IsFinite()) errors.Add(IMessageFactory.Factory(IMessageLevels.Error, $"The histogram bin is invalid because its midpoint value: {obj.MidPoint} defined by the equations: ( BinMax: {obj.Maximum} - BinMin: {obj.Minimum} ) / 2, is not a finite numerical value."));
            return errors;
        }
    }
}
