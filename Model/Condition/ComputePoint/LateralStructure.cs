...
namespace Model.ComputationPoint
{
    ...
    public class LateralStructure
    {
        ...
        public IFdaFunction CreateInteriorExteriorFunction(IFdaFunction exteriorStageFunction, bool hasFailureFunction)
        {
            //todo: Refactor: i reworked this a bit
            List<double> exteriorStages = new List<double>(); // (exteriorStageFunction.GetOrdinatesFunction( )).Function.YValues.Cast<double>().ToList();
            foreach(ICoordinate coord in exteriorStageFunction.Function.Coordinates)
            {
                exteriorStages.Add(coord.Y.Value());
            }
            ...
            return ImpactAreaFunctionFactory.Factory(exteriorStages, interiorStages,ImpactAreaFunctionEnum.ExteriorInteriorStage ); //new OrdinatesFunction(exteriorStages.ToArray( ), interiorStages.ToArray( ), FunctionTypes.ExteriorInteriorStage);
        }
        #endregion
    }
}
