    ...
    internal sealed class DamageValidator : IValidator<Damage>
    {
        ...
        public IEnumerable<IMessage> ReportErrors(Damage obj)
        {
            ...
            else
            {
                ...
                if (!(obj.Units == UnitsEnum.Dollars)) msgs.Add(IMessageFactory.Factory(IMessageLevels.Error, $"The specified units: {obj.Units.Print()} are not a valid measurement of flood damage. At this time the only supported unit of measurement for this {obj.ParameterType.Print()} parameter is: {obj.ParameterType.DefaultUnits().Print()}."));
